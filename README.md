...
- Компактный режим создания кастомных символов + приложение-редактор иконок
...
void createChar(uint8_t index, const uint64_t bitmap);         // символ из 8 строк по 5 бит
...
### UTF-8
UTF-8 версия декодирует однобайтовые и двухбайтовые символы, использует похожие символы из встроенной таблицы дисплея и при необходимости загружает глифы в ячейки кастомных символов. По умолчанию используются все 8 слотов. Количество слотов определяет, сколько уникальных UTF-8 символов, не совпадающих с английскими, может находиться на дисплее одновременно. При использовании поверх текущего лимита будут перезаписываться самые старые символы. Для использования этой версии совместно со своими кастомными символами нужно "ограничить" библиотеку при помощи `setGlyphSlots` - она использует ячейки от 0 до установленного количества минус один.

По умолчанию в библиотеке созданы таблицы для вывода кириллицы (RU+BY+UKR), а также корректно декодируется символ градуса.

```cpp
// UTF-8 использует 0..5, слоты 6..7 свободны для createChar
lcd.setGlyphSlots(6);
